#Windows Endpoint Disk Space Popup Alert

<#
.SYNOPSIS
This script is designed to check the remaining disk space on the C drive and alert the employee if it is below a minimum configured threshold.
For more information see: https://azuretothemax.net/2024/10/17/automating-disk-space-alerts-with-intune-proactive-remediations/

.Description
1. Load nessesary assemblies and hide our script window (not that it's visible when ran from Intune anyways)
2. Read our configurable variables
3. Declare our functions (some of which rely on the variables we just declared)
4. Create our logging directors and start logging
...
6. Pull the device disk space
7. Declare our windows form and all components
8. Determine if we need to now present that form (yes this order is weird, leave it be)
9. Throw the popup
10. The script ends when either the timeout completes or the user clicks the close button.
...
#You may want to comment this out for testing as well.
...
#Disk Space Minimum
	#The minimum amount of disk space in GB devices should have. Any value under this will produce an alert.
	$MinimumSpaceGB = "10"

	#Testing value
	#Use this to test fake values of your system drive (example: 5.0 with NO QUOTES) so you don't have to raise $MinimumSpaceGB to a stupidly high number for testing.
	#To disable, set to $null
	$TestingDiskSpaceValue = $null

#Text customization
<#
For the main message, see "$richTextBox1.Text"
For the help button message, see "$btn2.Text"
...
#Automatic Timeout
	...
	$TimeOutDuration = 2700 #default is 2700 (45 minutes). Do NOT set it higher than 55 minutes. When ran through ISE or visual studio code, this will tick down too fast.
...
#Branding and Customization
	...
	$MainFormBackgroundColor = [System.Drawing.Color]::FromArgb(255,0,3,32)
...
function button2_click {
	#What our More Information button does
	Add-Content "$($LogFileFolder)\$($LogFileName)" "$(get-date): $($env:USERNAME) Opening help page (clicked More Information button)." -Force
	Start-Process -FilePath "https://AzureToTheMax.net"
}
...
#Check free space - needs to be before the form is declared
Add-Content "$($LogFileFolder)\$($LogFileName)" "

$(get-date): Disk Space check running as $($env:USERNAME) on $($env:COMPUTERNAME)" -Force

Add-Content "$($LogFileFolder)\$($LogFileName)" "$(get-date): Timeout is set to -$($TimeOutDuration)- seconds. Timeout visibility is $($AutoCloseVisible)." -Force
...
#Region find disk space
#Find our C drive free disk space
	...
	$DriveStorage | ForEach-Object -Process {
		if ($_.DriveLetter -ne $null -and $_.DriveType -ne "CD-ROM" -and $_.DriveType -ne "Removable"){
			...
			if ($_.DriveLetter -eq "C"){
				$CDriveSpace = $SizeRemaining
				Add-Content "$($LogFileFolder)\$($LogFileName)" "$(get-date): C Drive has $($CDriveSpace)GB Free." -Force
			}
...
#Region testing
#This is used to override the free space we just calculated for testing puposes. See $TestingDiskSpaceValue in the variables region.
if ($null -ne $TestingDiskSpaceValue){
	Add-Content "$($LogFileFolder)\$($LogFileName)" "$(get-date): Testing value is enabled and set to: $($TestingDiskSpaceValue)" -Force
	Write-Warning "Testing value is enabled!"
	$CDriveSpace = $TestingDiskSpaceValue
}
#endregion
...
$MainForm.ClientSize = New-Object System.Drawing.Size(800, 450)
$MainForm.MaximumSize = New-Object System.Drawing.Size(800, 450)
$MainForm.MinimumSize = New-Object System.Drawing.Size(800, 450)
$MainForm.MaximizeBox = $false
$MainForm.MinimizeBox = $false
$MainForm.TopMost = $True
$MainForm.TopLevel = $True
$MainForm.StartPosition = "CenterScreen"
$MainForm.Text = "Disk Space Alert"


# see variables section to change me!
$MainForm.BackColor = $MainFormBackgroundColor
...
$btn1.Name = "btn1"
#$btn1.Location = New-Object System.Drawing.Point(125, 325)
$btn1.Location = New-Object System.Drawing.Point(425, 325)
...
# On click call function launch installer
$btn1.add_Click{
button_click
}
#endregion
...
#Region Help Button
#This defines our help button, where it is, and what options it has. What it does when clicked is defined by "function button2_click".
...
$btn2.Name = "btn2"
#$btn2.Location = New-Object System.Drawing.Point(125, 325)
$btn2.Location = New-Object System.Drawing.Point(100, 325)
$btn2.Size = New-Object System.Drawing.Size(250, 50)
$btn2.TabIndex = 0
$btn2.TabStop = $False
$btn2.Text = "More Information"
...
#You can use this if you want however, it requires an ugly full URL in the box and, you will have no logging that they clicked it. This is why we use a button to launch or URL instead.
...
$richTextBox1.Text = "This device only has $($CDriveSpace)GB of free disk space remaining. This is critically low and could lead to your device breaking. For guidance on how best to free up space, please click the More Information button below which will direct you to an article with additional instruction. Alternatively, you can call 1-XXX-XXX-XXXX or email help@contoso.com for assistance. You will continue to receive this warning until at least $($MinimumSpaceGB)GB of free disk space is available."
...
$pictureBox1.Location = New-Object System.Drawing.Point(((800/2)-($imageWidth/2)), 10) #To get a centered result, this is your page width (800 by default) dividied by two, minus half your image width.
...
#Create the actual form and label
...
#This must be stored as a label otherwise it will tick once and freeze. It is VERY imporant you declare this new object AFTER the form has been created.
...
$MainForm.Controls.Add($btn1)
$MainForm.Controls.Add($btn2)
...
if ($CDriveSpace -le $MinimumSpaceGB){
	Add-Content "$($LogFileFolder)\$($LogFileName)" "$(get-date): C Drive space free space of $($CDriveSpace)GB is less than the minimum of $($MinimumSpaceGB)GB - prompting." -Force
	main
} else {
	Add-Content "$($LogFileFolder)\$($LogFileName)" "$(get-date): C Drive space free space of $($CDriveSpace)GB is NOT less than the minimum of $($MinimumSpaceGB)GB - exiting." -Force
	write-host "$(get-date): C Drive space free space of $($CDriveSpace)GB is NOT less than the minimum of $($MinimumSpaceGB)GB - exiting."
